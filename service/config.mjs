...
export function defaultConfigDir() {
  if (process.platform === "darwin") {
    return path.join(os.homedir(), "Library", "Application Support", "claude-desktop-edamame");
  }
  if (process.platform === "win32") {
    return path.join(
      process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"),
      "claude-desktop-edamame",
    );
  }
  return path.join(process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config"), "claude-desktop-edamame");
}
...
export function defaultSystemctlCommand() {
  if (typeof process.env.CLAUDE_DESKTOP_EDAMAME_SYSTEMCTL_COMMAND === "string") {
    const override = process.env.CLAUDE_DESKTOP_EDAMAME_SYSTEMCTL_COMMAND.trim();
    if (override) return override;
  }
  return "systemctl";
}

function defaultCoworkSessionsRoot() {
  if (process.platform === "darwin") {
...
export const DEFAULT_CONFIG = Object.freeze({
  ...
  scopeProcessPaths: [],
  scopeParentPaths: [
    // Cowork is a mode of the Claude Desktop GUI app only (no standalone CLI).
    // macOS .app bundle
    "*/Claude.app/Contents/MacOS/Claude",
    "*/Claude.app/Contents/MacOS/*",
    // Windows (Programs, WindowsApps, MSIX) -- include backslash forms for raw L7 paths
    "*/Claude/Claude.exe",
    "*\\Claude\\Claude.exe",
    "*/Claude.exe",
    "*\\Claude.exe",
    ...
    "*\\AppData\\Local\\Programs\\Claude",
    // Linux: /opt, XDG, snap, flatpak, AppImage-style mounts
    "/opt/claude*",
    "*/.local/share/claude*",
    "/usr/bin/claude",
    ...
    "*/flatpak/",
    "/tmp/.mount_claude",
    // Node subprocess spawned by the Desktop app or MCP bridge
    "*/node",
    "*/node.exe",
    "*\\node.exe",
    // MCP bridge script spawned by the Claude Desktop package
...
function normalizeConfig(raw = {}, overrides = {}) {
  ...
  return {
    ...
    claudeDesktopLlmHosts: uniqueStrings(
      toArray(firstDefined(merged, "claudeDesktopLlmHosts", "claude_desktop_llm_hosts")).concat(DEFAULT_CONFIG.claudeDesktopLlmHosts),
    ),
...
export async function loadConfig(options = {}) {
  const configPath = path.resolve(expandHome(options.configPath || process.env.CLAUDE_DESKTOP_EDAMAME_CONFIG || defaultConfigPath()));
