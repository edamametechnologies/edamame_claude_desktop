...
} from "./claude_desktop_extrapolator.mjs";
...
export async function runHealthcheck(config, options = {}) {
  ...
  const extrapolatorState = await loadState(config, "claude-desktop-extrapolator", null);
  if (extrapolatorState) {
    const lastError =
      typeof extrapolatorState.lastError === "string" && extrapolatorState.lastError.trim()
        ...
        : null;
    addCheck("claude_desktop.extrapolator", !lastError, {
