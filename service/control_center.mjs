...
import { runLatestExtrapolation } from "./claude_desktop_extrapolator.mjs";
...
function compactHealthDetail(name, detail) {
  ...
  if (name === "claude_desktop.extrapolator") {
...
function baselineConfigJson(config) {
  return {
    ...
    claude_desktop_llm_hosts: config.claudeDesktopLlmHosts,
...
function summaryLine({ pairingConfigured, posture, health, hostActionResult, refreshResult, extrapolator }) {
  ...
  if (!pairingConfigured) return "Pair the local Claude Desktop bridge with EDAMAME to enable status reads.";
  const authFailure =
...
export async function buildControlCenterPayload(config, options = {}) {
  ...
  const extrapolatorState = compactExtrapolatorState(
    await loadState(config, "claude-desktop-extrapolator", {}),
  );
...
export async function applyPairing(config, args = {}) {
  ...
  try {
    const extState = await loadState(config, "claude-desktop-extrapolator", {});
    if (
      ...
      await saveState(config, "claude-desktop-extrapolator", extState);
    }
  } catch (_e) {
...
export async function requestAppPairing(config, args = {}) {
  ...
  const body = {
    client_name: String(args.client_name || config.clientName || "EDAMAME for Claude Desktop").trim(),
    agent_type: String(config.agentType || "claude_desktop"),
