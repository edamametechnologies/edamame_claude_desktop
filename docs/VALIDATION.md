...
| Raw ingest E2E | Inject synthetic Claude Desktop transcripts and poll the merged behavioral model | `bash tests/e2e_inject_intent.sh` | every injected `session_key` appears under the correct `agent_type=claude_desktop` and `agent_instance_id` |
...
| Restart recovery | Restart the EDAMAME host with an empty remote model store | workstation exercise | the next Claude Desktop refresh restores or repushes the local contributor slice |
