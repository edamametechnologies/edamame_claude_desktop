#!/usr/bin/env bash
# End-to-end local test: provision check, three synthetic Claude Desktop transcripts (API / shell / git),
...
#   CLAUDE_DESKTOP_EDAMAME_CONFIG  Override path to claude-desktop-edamame config.json
#   E2E_SKIP_PLUGIN_CHECK       If set to 1, skip Claude Desktop marketplace / MCP path checks
...
REPO_ROOT="${CLAUDE_DESKTOP_REPO_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}"
...
CONFIG_PATH="${CLAUDE_DESKTOP_EDAMAME_CONFIG:-$CONFIG_HOME/config.json}"
MCP_SNIPPET_PATH="$CONFIG_HOME/claude-desktop-mcp.json"
...
if [[ "${E2E_SKIP_PROVISION_STRICT:-0}" != "1" ]]; then
  ...
  if [[ ! -f "$INSTALL_ROOT/service/claude_desktop_extrapolator.mjs" ]]; then
    echo "FAIL: installed package missing: $INSTALL_ROOT/service/claude_desktop_extrapolator.mjs" >&2
    echo "Run from edamame_claude_desktop: bash setup/install.sh <workspace_root>" >&2
  ...
  REPO_PLG="$REPO_ROOT/.claude-plugin/plugin.json"
  INS_PLG="$INSTALL_ROOT/.claude-plugin/plugin.json"
...
if vpl != vil:
    print(f"FAIL: .claude-plugin/plugin.json version mismatch repo={vpl} installed={vil}", file=sys.stderr)
    raise SystemExit(1)
if vp != vpl:
...
PY
  ...
  if [[ ! -f "$MCP_SNIPPET_PATH" ]]; then
    echo "FAIL: Claude MCP snippet missing: $MCP_SNIPPET_PATH (re-run setup/install.sh)" >&2
    exit 1
  fi
  if ! grep -q "claude_desktop_edamame_mcp" "$MCP_SNIPPET_PATH" 2>/dev/null; then
    echo "FAIL: $MCP_SNIPPET_PATH must reference claude_desktop_edamame_mcp.mjs" >&2
    exit 1
  fi
...
if [[ ! -f "$PACKAGE_ROOT/service/claude_desktop_extrapolator.mjs" ]]; then
  echo "FAIL: claude_desktop_extrapolator.mjs not found under $PACKAGE_ROOT/service" >&2
  exit 1
fi
...
if [[ "${E2E_SKIP_PLUGIN_CHECK:-0}" == "1" ]]; then
  echo "SKIP: Claude Desktop plugin/MCP verification (E2E_SKIP_PLUGIN_CHECK=1)"
...
" 2>/dev/null; then
    echo "OK: Claude Desktop settings show an edamame plugin entry"
  ...
  if [[ "${E2E_SKIP_PROVISION_STRICT:-0}" != "1" ]]; then
    echo "OK: no ~/.claude/settings.json; MCP snippet already validated in strict step"
  elif [[ ! -f "$MCP_SNIPPET_PATH" ]]; then
    echo "FAIL: no ~/.claude/settings.json and no MCP snippet at $MCP_SNIPPET_PATH" >&2
    echo "      Run setup/install.sh or enable the marketplace plugin." >&2
    exit 1
  else
    echo "OK: using MCP snippet (no Claude Desktop settings.json)"
  fi
fi
...
echo "=== Step 2: Write synthetic transcripts (Desktop session structure) ==="
...
agent_type = (cfg.get('agent_type') or cfg.get('agentType') or 'claude_desktop').strip()
agent_id = (cfg.get('agent_instance_id') or cfg.get('agentInstanceId') or '').strip()

cowork_root_raw = cfg.get('cowork_sessions_root') or cfg.get('coworkSessionsRoot') or ''
if cowork_root_raw:
    cowork_root = Path(os.path.expanduser(str(cowork_root_raw))).resolve()
...
transcript_dir = cowork_root / session_uuid / '.claude' / 'projects' / f'{ws.name}-edamame-e2e-inject'
...
for ((ea = 1; ea <= E2E_EXTRAP_MAX_ATTEMPTS; ea++)); do
  ...
  set +e
  node "$PACKAGE_ROOT/service/claude_desktop_extrapolator.mjs" --config "$CONFIG_PATH" --json >"$EXTRAP_LOG" 2>&1
  EX_CODE=$?
  set -e
  cat "$EXTRAP_LOG"
  if [[ "$EX_CODE" != 0 ]]; then
...
export _E2E_DIAG_SUITE="claude_desktop"
