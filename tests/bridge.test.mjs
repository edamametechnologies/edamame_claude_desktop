...
import {
  ...
  createClaudeDesktopDrivenRefresh,
  handleRequest,
  tryExtractMessages,
} from "../bridge/claude_desktop_edamame_mcp.mjs";
...
async function makeBridgeFixture(options = {}) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "claude-desktop-edamame-bridge-"));
  const workspaceRoot = path.join(root, "edamame_project");
  const coworkSessionsRoot = path.join(root, "cowork-sessions");
  const transcriptDir = path.join(coworkSessionsRoot, "fixture-workspace");
    ...
    JSON.stringify(
      {
        ...
        cowork_sessions_root: coworkSessionsRoot,
...
`,
  ...
  return {
    ...
    agentInstanceId: "claude-desktop-bridge-test",
    hostKind,
    ...
    claudeDesktopLlmHosts: ["api.anthropic.com:443"],
...
test("handleRequest returns MCP initialize and tool list responses", async () => {
  ...
  assert.ok(
    toolsResponse.result.tools.some((tool) => tool.name === "claude_desktop_refresh_behavioral_model"),
  );
  const controlCenterTool = toolsResponse.result.tools.find(
    (tool) => tool.name === "edamame_claude_desktop_control_center",
  );
  assert.ok(controlCenterTool, "control center tool should be defined");
  assert.equal(
    toolsResponse.result.tools.some((tool) => tool.name === "edamame_claude_desktop_control_center_apply_pairing"),
    true,
  );
  assert.equal(
    toolsResponse.result.tools.some((tool) => tool.name === "edamame_claude_desktop_control_center_run_host_action"),
    true,
  );
...
test("bridge can dispatch dry-run extrapolation and healthcheck tools", async () => {
  ...
  const extrapolatorResponse = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "claude_desktop_refresh_behavioral_model",
  ...
  assert.equal(extrapolatorPayload.agentType, "claude_desktop");
  assert.equal(extrapolatorPayload.agentInstanceId, "claude-desktop-bridge-test");
  assert.equal(extrapolatorPayload.rawSessions.agent_type, "claude_desktop");
  assert.equal(extrapolatorPayload.rawSessions.agent_instance_id, "claude-desktop-bridge-test");
  assert.equal(extrapolatorPayload.rawSessions.source_kind, "claude_desktop");
  ...
  const healthResponse = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "claude_desktop_healthcheck",
...
test("control center pairing stores PSK and updates config", async () => {
  ...
  const response = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "edamame_claude_desktop_control_center_apply_pairing",
      arguments: {
...
test("control center can auto-pair a local posture host", async () => {
  ...
  const response = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "edamame_claude_desktop_control_center_run_host_action",
      arguments: {
...
test("claude-desktop-driven refresh skips recent persisted runs", async () => {
  const refresh = createClaudeDesktopDrivenRefresh(
    { divergenceIntervalSecs: 120 },
    {
      loadState: async () => ({
...
test("bridge can dispatch the control center tool with structured content", async () => {
  ...
  const response = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "edamame_claude_desktop_control_center",
      arguments: {},
    },
  });
...
test("legacy control center aliases are rejected", async () => {
  ...
  const dottedAliasResponse = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "edamame.claude_desktop_control_center",
      arguments: {},
    },
  });

  assert.equal(dottedAliasResponse.error.message, "unsupported_tool:edamame.claude_desktop_control_center");
  ...
  const response = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "claude_desktop.control_center",
      arguments: {},
    },
  });

  assert.equal(response.error.message, "unsupported_tool:claude_desktop.control_center");
});

test("handleRequest uses Claude Desktop lifecycle refresh hooks", async () => {
  const config = await makeBridgeFixture();
  const calls = [];
  const autoRefresh = {
  ...
  assert.deepEqual(calls, [
    ...
    { type: "maybeRun", trigger: "tool_call:claude_desktop_healthcheck" },
  ]);
});
...
test("claude-desktop-driven refresh reruns when transcript changed after recent persisted run", async () => {
  const now = Date.now();
  ...
  const result = await runLatestExtrapolation(config, {
    buildPayload: async () => ({
      ...
      rawSessions: {
        ...
        agent_instance_id: "claude-desktop-bridge-test",
        source_kind: "claude_desktop",
        sessions: [{ session_key: "session-stable" }],
      },
      rawPayloadHash: "payload-123",
    }),
    loadState: async (_cfg, name) => {
      assert.equal(name, "claude-desktop-extrapolator");
      return {
...
test("runLatestExtrapolation pushes heartbeat window when no sessions and no cached window", async () => {
  ...
  assert.equal(heartbeat.agent_type, "claude_desktop");
  assert.equal(heartbeat.agent_instance_id, "claude-desktop-bridge-test");
  ...
  assert.equal(savedStates[0].lastGeneratedWindow.agent_type, "claude_desktop");
});
