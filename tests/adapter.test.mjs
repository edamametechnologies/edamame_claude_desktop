...
async function makeTempFixture() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "claude-desktop-edamame-adapter-"));
...
test("buildRawSessionIngestPayload forwards transcript context with derived session hints", async () => {
  ...
  const result = await buildRawSessionIngestPayload(
    makeBaseConfig(fixture, {
      scopeParentPaths: ["*/Claude*", "*/claude_desktop_edamame_mcp.mjs"],
    }),
  );

  assert.equal(result.rawSessions.agent_type, "claude_desktop");
  assert.equal(result.rawSessions.agent_instance_id, "claude-desktop-test-fixture");
  assert.equal(result.rawSessions.source_kind, "claude_desktop");
...
test("collectTranscriptSessions extracts tools and commands from JSONL assistant text", async () => {
  ...
  const sessions = await collectTranscriptSessions(
    makeBaseConfig(fixture, { claudeDesktopLlmHosts: [] }),
  );
