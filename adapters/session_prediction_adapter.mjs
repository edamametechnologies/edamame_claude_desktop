...
function looksLikeClaudeDesktopProjectPath(filePath, hints) {
...
async function candidateTranscriptFiles(config) {
  ...
  const hints = config.transcriptProjectHints || [];
  const resolved = [];
  ...
  for (const { root, sourceKind } of roots) {
    ...
    const hintCandidates = allFiles.filter((filePath) =>
      looksLikeClaudeDesktopProjectPath(filePath, hints),
    );
    ...
    for (const filePath of candidates) {
      ...
      resolved.push({ filePath, stat, sourceKind });
    }
  }
...
function extractTraffic(text, commands, config) {
  ...
  for (const host of config.claudeDesktopLlmHosts || []) {
    if (host.includes(":")) {
      hosts.push(host);
    } else {
      hosts.push(`${host}:443`);
...
function extractToolNamesFromProse(text) {
  ...
  const names = [];
  for (const toolName of CLAUDE_CODE_TOOL_NAMES) {
...
export async function collectTranscriptSessions(config, options = {}) {
  ...
  for (const { filePath, stat, sourceKind } of candidates) {
    const sessionId = transcriptSessionId(filePath);
    const current = grouped.get(sessionId) || {
      sessionId,
      sourceKind: sourceKind || "claude_desktop",
      txtPath: null,
      txtMtimeMs: 0,
      jsonlPath: null,
      jsonlMtimeMs: 0,
  ...
  for (const candidate of limited) {
    ...
    records.push({
      ...
      title: firstNonEmptyLine(userText) || `Claude Desktop session ${candidate.sessionId}`,
...
export async function buildRawSessionIngestPayload(config, options = {}) {
  ...
  const agentType = config.agentType || "claude_desktop";
  const agentInstanceId = config.agentInstanceId || "claude-desktop-default";
  ...
  const rawSessions = {
    ...
    sessions: sessions.map((session) => {
      ...
      return {
        ...
        derived_expected_traffic: uniqueStrings([...derivedExpectedTraffic, ...(config.claudeDesktopLlmHosts || [])]),
